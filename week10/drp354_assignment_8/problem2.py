...
def main(filename):
  ...
  #calculating nbin
  q75, q25 = np.percentile(date2num(timeStamp),[75,25])
  iqr = q75 - q25
  n = len(timeStamp)
  w = 2.0*iqr*(n**(-1.0/3.0))
  deltaDateNum = date2num(maxDate)-date2num(minDate)
  nbin = int(deltaDateNum/w)
  #tickname adjustment
  deltaDate = maxDate - minDate
  #nbin = int(deltaDate.days)
  ind = [date2num(minDate + timedelta(days=x)) for x in range(0, 99,14)]
  dateList = [(minDate + timedelta(days=x)).strftime("%Y-%m-%d") for x in range(0, 99, 14)]
  ...
  plt.xticks(ind,dateList, rotation="horizontal")
